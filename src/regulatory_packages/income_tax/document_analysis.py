"""Deep structured analysis of Income Tax instrument text for ontology and LLM export."""
...
from regulatory_packages.income_tax.taxonomy import infer_assessees_from_text, infer_topics
...
_CROSS_REF_RE = re.compile(
    r"(?:Circular|Notification|Order|Rule)s?\s+(?:No\.?|:)\s*([A-Z0-9/\-_.]+)|"
    r"(?:Income[- ]tax Act,?\s*1961)|"
    r"(?:Finance Act,?\s*\d{4})|"
    r"(?:section|rule|clause|sub[- ]section)\s+(\d+[A-Za-z]?(?:\([a-z0-9]+\))?)",
    re.I,
)
...
def _top_terms(text: str, limit: int = 20) -> list[str]:
    stop = {
        "income", "tax", "india", "department", "cbdt", "circular", "notification",
        "section", "rule", "shall", "that", "this", "with", "from", "under", "which",
        "such", "have", "been", "will", "their", "there", "where", "when", "also",
    }
...
def analyze_document_text(
    ...
    *,
    ...
    for match in _CROSS_REF_RE.finditer(joined):
        ...
        seen_refs.add(key)
        lower_ref = ref.lower()
        if "income" in lower_ref and "act" in lower_ref:
            kind = "act"
        elif "finance act" in lower_ref:
            kind = "finance_act"
        elif ref[:1].isdigit():
            kind = "section"
        else:
    ...
    text_entities = infer_assessees_from_text(f"{title} {text[:25000]}")
