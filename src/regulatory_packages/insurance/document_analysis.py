"""Deep structured analysis of IRDAI insurance instrument text for ontology and LLM export."""
...
from regulatory_packages.insurance.taxonomy import infer_entities_for_document, infer_topics
...
_CROSS_REF_RE = re.compile(
    r"(?:Circular|Regulation|Notification|Guideline|Order)s?\s+(?:No\.?|:)\s*([A-Z0-9/\-_. ]{4,80})|"
    r"(Insurance Act,?\s*1938|IRDA Act,?\s*1999|Insurance Regulatory and Development Authority Act)|"
    r"(?:section|regulation|rule|clause)\s+(\d+[A-Za-z]?(?:\([a-z0-9]+\))?)",
    re.I,
)
...
_HEADING_RE = re.compile(
    r"(?:^|\n)\s*(?:CHAPTER|Chapter|PART|Part|Regulation|REGULATION|Schedule|SCHEDULE)\s+([IVXLC\d]+)[:\.\-\s]+([^\n]{5,120})",
    re.M,
)
...
def _top_terms(text: str, limit: int = 20) -> list[str]:
    stop = {
        "irdai", "insurance", "insurer", "policy", "regulation", "circular",
        "shall", "that", "this", "with", "from", "under", "which", "such",
        "authority", "company", "general", "life",
    }
...
def analyze_document(
    ...
    for match in _CROSS_REF_RE.finditer(joined):
        ...
        if "act" in lower:
            kind = "act"
        elif "regulation" in lower or "circular" in lower or "guideline" in lower:
            kind = "instrument"
        elif ref[:1].isdigit():
            kind = "section"
        else:
    ...
    return {
        ...
        "topicsFromText": infer_topics(title, f"{summary} {text[:20000]}")[:6],
